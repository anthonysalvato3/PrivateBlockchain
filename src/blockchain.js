...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //Set previous block hash (excluding genesis block)
            if (block.height > 0) {
                let previousBlockHeight = self.height;
                let previousBlock = self.chain[previousBlockHeight];
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //Check time elapsed < 5 minutes
            if (currentTime - messageTime < FIVE_MINUTES) {
                try {
                    //Verify message with signature
                    bitcoinMessage.verify(message, address, signature)

                    //Construct data for block and add to chain
                    let blockData = new Object();
                    blockData.owner = address;
                    blockData.star = star;
                ...
                reject('Time for signature has surpassed the 5 minute expiration.');
            }
        });
    }
            ...
            self.chain.forEach(async block => {
                ...
                if (block.height > 0) {
                    ...
                    if (block.previousBlockHash !== previousBlock.hash) {
                        errorLog.push('Block ' + block.hash + ' is detached from previous block.');
                    }
                }
            })
            resolve(errorLog);
